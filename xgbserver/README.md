...
This rock can be tested locally by building it from source and running it following the [upstream usage example](https://kserve.github.io/website/docs/model-serving/predictive-inference/frameworks/xgboost):
...
1. Export your rock to Docker's image registry:
    ```bash
    sudo rockcraft.skopeo --insecure-policy copy oci-archive:${rock_filename} docker-daemon:xgbserver:local
    ```
...
1. Launch the server:
    ...
    sudo docker run -p 8080:8080 -v ./sample_model:/mnt/models xgbserver:local --model_name test_model --model_dir=/mnt/models --http_port=8080
    ```
